...
async function run() {
  ...
  console.log('addresses.Universe', addresses.Universe)
  // console.log('addresses.Universe', web3.utils.toChecksumAddress(addresses.Universe))
  ...
  const amountRemaining = await trade.call({ from: otherAccount, gas: 2000000, value: web3.utils.toWei('.001') });
  console.log(`Amount remaining from fill: ${amountRemaining}`);
  const tradeTx = await trade.send({ from: otherAccount, gas: 2000000, value: web3.utils.toWei('.001') });
